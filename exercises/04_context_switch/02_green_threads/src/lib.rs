...
impl Scheduler {
    ...
    fn schedule_next(&mut self) {
        ...
        let current_thread = &mut self.threads[current_idx];
        ...
        let current_ctx = self.threads.get_mut(current_idx).unwrap().ctx.as_mut_ptr();
        let next_ctx = self.threads.get_mut(next_idx).unwrap().ctx.as_ptr();
        unsafe { switch_context(&mut *current_ctx, &*next_ctx); }
    }
}
