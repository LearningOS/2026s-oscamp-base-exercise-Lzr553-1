...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        for level in (1..=2).rev() {
            ...
            let pte = self.nodes[&cur_ppn].entries[vpn];
            if (pte & PTE_V) == 0 {
                let new_ppn = self.alloc_node();
                let cur_node = self.nodes.get_mut(&cur_ppn).unwrap();
                cur_node.entries[vpn] = (new_ppn << PPN_SHIFT) | PTE_V;
            }
            cur_ppn = self.nodes[&cur_ppn].entries[vpn] >> PPN_SHIFT;
        }

        let vpn0 = Self::extract_vpn(va, 0);
        let node0 = self.nodes.get_mut(&cur_ppn).unwrap();
        node0.entries[vpn0] = ((pa >> 12) << PPN_SHIFT) | PTE_V | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        for level in (0..=2).rev() {
            ...
            if level == 1 && (pte & (PTE_R | PTE_W | PTE_X)) != 0 {
                let super_offset = va & 0x1FFFFF;
                return TranslateResult::Ok((cur_ppn << 12) | super_offset);
            }
        }
        let offset = va & (PAGE_SIZE as u64 - 1);
        TranslateResult::Ok((cur_ppn << 12) | offset)
    }
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let pte2 = self.nodes[&cur_ppn].entries[vpn2];
        if (pte2 & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
            let cur_node = self.nodes.get_mut(&cur_ppn).unwrap();
            cur_node.entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
        }
        cur_ppn = self.nodes[&cur_ppn].entries[vpn2] >> PPN_SHIFT;

        let vpn1 = Self::extract_vpn(va, 1);
        let node1 = self.nodes.get_mut(&cur_ppn).unwrap();
        node1.entries[vpn1] = ((pa >> 12) << PPN_SHIFT) | PTE_V | flags;
    }
}
